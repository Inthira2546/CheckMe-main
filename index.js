...
const app = Vue.createApp({
    ...
    methods: {
        fetchAdditionalUserData(email) {
            // ทำการค้นหาข้อมูลของผู้ใช้เพิ่มเติมจาก Firebase Firestore
            db.collection("CollegeStudents").where("email", "==", email).get()
                .then((querySnapshot) => {
                    if (!querySnapshot.empty) {
                        // ถ้าพบข้อมูลผู้ใช้ใน Firestore
                        // ดึงข้อมูลและทำการใช้งานตามต้องการ
                        querySnapshot.forEach((doc) => {
                            const userData = doc.data();
                            console.log("Additional user data:", userData);
    
                            // ทำอะไรสักอย่างเมื่อค้นหาข้อมูลผู้ใช้เพิ่มเติมสำเร็จ
                            // ยกตัวอย่างเช่น:
                            // this.additionalUserData = userData;
                        });
                    } else {
                        // ถ้าไม่พบข้อมูลผู้ใช้ใน Firestore
        ...
        //         .then((querySnapshot) => {
        //             if (!querySnapshot.empty) {
        //                 // มีในฐานข้อมูลเป็น Teacher
        //                 this.userRole = 'Teacher';
        //             } else {
        //                 // ไม่มีในฐานข้อมูล Teachers
        //                 // ต้องเช็คในฐานข้อมูล Students
        //                 db.collection("CollegeStudents").where("email", "==", email).get()
        //                     .then((studentQuerySnapshot) => {
        //                         if (!studentQuerySnapshot.empty) {
        //                             // มีในฐานข้อมูลเป็น Student
        //                             this.userRole = 'Student';
        //                             // ทำการล็อกอินหรือดำเนินการต่อไปตามที่คุณต้องการ
        //                             // เช่น เรียกฟังก์ชันหรือส่งค่ากลับไปยังหน้าที่ต้องการ
        //                         } else {
        //                             // ไม่มีในฐานข้อมูล Students หรืออาจจะเกิดข้อผิดพลาดอื่น ๆ
        //                             console.log("User not found in Teachers or Students collection");
        //                         }
        //                     })
        //                     .catch((error) => {
        //                         console.error("Error checking user role in Students collection:", error);
        //                     });
        //             }
        //         })
        //         .catch((error) => {
        //             console.error("Error checking user role in Teachers collection:", error);
        //         });
        // },
